// hooks/useRobotMovementV4.js
import { useRef, useEffect } from "react";
import { useFrame } from "@react-three/fiber";
import * as THREE from "three";
import { useKeyboardMovementV4 } from "./useKeyboardMovementV4";
import { useNetworkedPlayerV4 } from "./useNetworkedPlayerV4";
...
/**
 * Hook for controlling the local player movement in bomb game.
 * Uses separate group "bombGamePlayerMovement" to avoid conflicts.
 */
export function useRobotMovementV4(ref) {
  const { computeMovement, direction } = useKeyboardMovementV4();
  const { broadcastTransform } = useNetworkedPlayerV4("bombGamePlayerMovement");
