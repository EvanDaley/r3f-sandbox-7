...
export function useNetworkedPlayerV4(group = "bombGamePlayerMovement") {
  const isHost = usePeerStore((s) => s.isHost);
  const peerId = usePeerStore((s) => s.peerId);
  const setPlayerTransform = usePlayerStoreV4((s) => s.setPlayerTransform);
  const getPlayers = usePlayerStoreV4.getState;
  ...
  useEffect(() => {
    ...
    const unsubscribe = messageBus.subscribe(group, ({ fromPeerId, payload, type }) => {
      ...
      switch (type) {
        ...
        case "playerListResponse": {
          ...
          console.log("✅ [Bomb Game] Synced player list from host:", players);
          break;
        }
    ...
    if (!isHost) {
      setTimeout(() => {
        ...
        console.log("📡 [Bomb Game] Requested player list from host");
      }, 200);
    }
