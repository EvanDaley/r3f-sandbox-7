  ...
  useFrame(() => {
    ...
    if (now - lastAutoReleaseCheck.current >= 200) {
      Object.entries(objects).forEach(([objectId, object]) => {
        if (!object.position || object.type !== 'bomb') return;
    ...
    if (keys.current[INTERACTION_KEY]) {
      // First, check if player is holding any bomb - if so, allow release
      let isHoldingAnyBomb = false;
      let heldBombId = null;
      Object.entries(objects).forEach(([objectId, object]) => {
        if (object.heldBy?.includes(peerId)) {
          isHoldingAnyBomb = true;
          heldBombId = objectId;
        }
      });

      // If player is holding a bomb, allow release from any distance
      if (isHoldingAnyBomb && heldBombId) {
        broadcastRelease(heldBombId, peerId);
        lastInteractionTime.current = now;
        return;
      }

      // If player is already holding a bomb, don't allow grabbing another
      if (isHoldingAnyBomb) {
        lastInteractionTime.current = now;
        return;
      }
      ...
      Object.entries(objects).forEach(([objectId, object]) => {
        ...
        if (distance <= INTERACTION_DISTANCE) {
          // Try to grab (only if not at capacity - max 2 players for bombs)
          const maxHolders = 2; // Bombs require exactly 2 players
