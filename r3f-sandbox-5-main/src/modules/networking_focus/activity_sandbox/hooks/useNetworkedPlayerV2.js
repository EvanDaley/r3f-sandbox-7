...
export function useNetworkedPlayerV2(group = "activityPlayerMovement") {
  const isHost = usePeerStore((s) => s.isHost);
  const peerId = usePeerStore((s) => s.peerId);
  const setPlayerTransform = usePlayerStoreV2((s) => s.setPlayerTransform);
  const getPlayers = usePlayerStoreV2.getState;
  ...
  useEffect(() => {
    ...
    const unsubscribe = messageBus.subscribe(group, ({ fromPeerId, payload, type }) => {
      ...
      switch (type) {
        ...
        case "playerListResponse": {
          ...
          console.log("✅ [Activity Sandbox] Synced player list from host:", players);
          break;
        }
    ...
    if (!isHost) {
      setTimeout(() => {
        ...
        console.log("📡 [Activity Sandbox] Requested player list from host");
      }, 200);
    }
